...
class EVRouteOptimizer:
    ...
    def get_vehicle_names(self):
        """Restored for app.py and index.html dropdown"""
        return sorted(self.ev_profiles.keys())
    ...
    # ------------------------------------------------------------------
    # Step 1: Core Physics Base (Segment Level)
    # ------------------------------------------------------------------

    def _segment_physics_kwh(self, car, total_mass, dist_m, speed_ms, grade_percent):
        """Pure Newtonian physics calculations for a single road segment"""
        grade_rad = math.atan(grade_percent / 100.0)
        ...
        # Aerodynamic Drag
        F_drag = 0.5 * self.AIR_DENSITY * car.get("drag_cd", 0.32) * car.get("frontal_area_m2", 2.45) * (speed_ms ** 2)
        E_drag = F_drag * dist_m
        ...
        E_gravity = F_gravity * dist_m
        if E_gravity < 0:
            E_gravity *= car.get("regen_efficiency", 0.65)

        E_total_joules = E_rolling + E_drag + E_gravity
        if E_total_joules > 0:
            E_total_joules /= car.get("drivetrain_efficiency", 0.85)

        return E_total_joules / 3600000.0

    # ------------------------------------------------------------------
    # Step 2: Hybrid Energy Calculator
    # ------------------------------------------------------------------

    def calculate_smart_energy_segmented(self, segments, total_duration_min, total_distance_km, vehicle_name, passenger_count, is_urban=False):
        """
        Called by app.py: Calculates the expected energy by laying empirical 
        urban/congestion multipliers on top of the exact segment physics.
        """
        car = self.ev_profiles.get(vehicle_name, self._fallback_profile)
        total_mass = car["base_mass_kg"] + (passenger_count * 75)

        # 1. Infer JS Traffic variables from the "Urban" flag
        slow_share = 0.40 if is_urban else 0.10
        crawl_share = 0.12 if is_urban else 0.02
        stop_go_factor = 1.14 if is_urban else 0.95
        cabin_load_kw = 1.30 if is_urban else 0.70  # AC runs harder/longer in city traffic

        # 2. Sum up the pure physics of the route
        raw_physics_kwh = 0.0
        for seg in segments:
            dist_m = seg["distance_km"] * 1000
            if dist_m == 0: continue
            
            # Estimate segment speed
            seg_duration_min = (seg["distance_km"] / total_distance_km) * total_duration_min if total_distance_km > 0 else 0
            base_speed_kmh = (seg["distance_km"] / seg_duration_min) * 60 if seg_duration_min > 0 else 30
            speed_ms = max(16, base_speed_kmh) / 3.6

            raw_physics_kwh += self._segment_physics_kwh(car, total_mass, dist_m, speed_ms, seg.get("grade_percent", 0.0))

        base_drive_kwh = raw_physics_kwh

        # 3. Apply Empirical Congestion Penalty (from JS logic)
        congestion_penalty_kwh = base_drive_kwh * ((0.85 * slow_share) + (0.55 * crawl_share)) * stop_go_factor

        # 4. Apply Cabin AC Load
        cabin_kwh = cabin_load_kw * (total_duration_min / 60.0)

        # 5. Final Expected Energy
        expected_kwh = base_drive_kwh + congestion_penalty_kwh + cabin_kwh

        # Minimum floor constraint
        expected_floor = base_drive_kwh * 0.88
        expected_kwh = max(expected_kwh, expected_floor)

        return round(expected_kwh, 4)

    def calculate_smart_energy(self, distance_km, duration_min, vehicle_name, passenger_count, grade_percent=0.0, is_urban=False):
        """Fallback for routes without detailed segment data"""
        segments = [{"distance_km": distance_km, "grade_percent": grade_percent}]
        return self.calculate_smart_energy_segmented(segments, duration_min, distance_km, vehicle_name, passenger_count, is_urban)

    # ------------------------------------------------------------------
    # Step 3: Uncertainty & Risk Layer
    # ------------------------------------------------------------------

    def calculate_uncertainty_metrics(self, expected_kwh, distance_km, duration_min, grade_percent, is_urban, battery_kwh, soc_percent):
        """
        Called by app.py: Computes risk reserve, protected arrival, and trip success probability.
        """
        available_kwh = battery_kwh * (soc_percent / 100.0)
        ...
        slow_share = 0.40 if is_urban else 0.10
        crawl_share = 0.12 if is_urban else 0.02
        exposure = 1.35 if is_urban else 0.90
        uncertainty_bias = 0.18 if is_urban else 0.05
        risk_bias = 0.0 # Assuming neutral driver

        # Reserve Energy (Worst-case scenario)
        reserve_multiplier = 0.08 + (1.35 * slow_share) + (0.95 * crawl_share)
        confidence_reserve_kwh = expected_kwh * reserve_multiplier * (0.58 + (0.34 * exposure)) * (1 + uncertainty_bias + risk_bias)

        robust_kwh = expected_kwh + confidence_reserve_kwh

        # Arrival Outcomes
        arrival_pct = ((available_kwh - expected_kwh) / battery_kwh) * 100
        arrival_safe_pct = ((available_kwh - robust_kwh) / battery_kwh) * 100
        uncertainty_gap_pct = max(0, arrival_pct - arrival_safe_pct)

        # Success Probability Index
        risk_index = 44 + \
            ((exposure - 1) * 20) + \
            (uncertainty_bias * 120) + \
            (risk_bias * 55) + \
            (max(0, 8 - arrival_safe_pct) * 3.2) + \
            (uncertainty_gap_pct * 1.5) - \
            (max(arrival_safe_pct, 0) * 1.12)

        risk_index = max(4, min(risk_index, 92))
        trip_success_pct = max(8, min(100 - risk_index, 98))
        ...
        return {
            "confidence_reserve_kwh": round(confidence_reserve_kwh, 4),
            "robust_kwh":             round(robust_kwh, 4),
            "available_kwh":          round(available_kwh, 3),
            "arrival_soc_pct":        round(arrival_pct, 1),
            "protected_arrival_pct":  round(arrival_safe_pct, 1),
            "trip_success_pct":       round(trip_success_pct, 0),
            "risk_level":             risk_level,
            "uncertainty_factor_pct": round((confidence_reserve_kwh / expected_kwh) * 100, 1) if expected_kwh > 0 else 0
        }

    # ------------------------------------------------------------------
    # Step 4: Route Flip Threshold
    # ------------------------------------------------------------------

    def find_flip_threshold(self, routes, vehicle_name, passenger_count, is_urban, battery_kwh):
        """Scans SOC to find the battery level where the recommendation changes."""
        def best_route_at_soc(soc):
            best = None
            best_score = float('inf')
            for r in routes:
